...
public class InternalLoggerTesting
{
    ...
    public async Task T0001_CRUD_Operations_ASYNC(string title, string message, LogTypes logType, string? addtlInformation = null)
    {
        try
        {
            // Add
    ...
    [InlineData("Test 009", "Message 009", LogTypes.Critical)]
    [InlineData("Test 010", "Message 010", LogTypes.Information)]
    public void T0002_CRUD_Operations(string title, string message, LogTypes logType, string? addtlInformation = null)
    {
        try
        {
            // Add
            var log = new InternalLogClone(title, message, logType, addtlInformation);
            var result = InternalLoggerService.SaveNewLog(log);
            log.Id = result;
            Assert.NotNull(log);
            Assert.True(result > 0);
            ...
            // Update
            log.Title = log.Title + " ::: Updated";
            var updateResult = InternalLoggerService.UpdateLog(log);
            Assert.NotNull(log);
            Assert.True(updateResult == result);

            // Delete
            var deleteResult = InternalLoggerService.DeleteLog(log);
            Assert.True(deleteResult == result);
        }
        catch (Exception ex)
        {
